...
const {
  ...
  clearHistory,
  formatTimestamp
} = useGeneratorHistory('youtube-description');
...
const formCompletion = computed(() => {
  const fields = [
    { name: 'Video Title', filled: !!videoTitle.value?.trim() },
    { name: 'Video Content', filled: !!videoContent.value?.trim() },
    { name: 'Timestamps', filled: !!timestamps.value }
  ];
  ...
  try {
    ...
    if (generatedContent.value) {
      addToHistory({
        ...
        preview: generatedContent.value.substring(0, 50) || 'Generated YouTube description'
      });
    }
