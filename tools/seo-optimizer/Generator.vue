...
const {
  ...
  clearHistory,
  formatTimestamp
} = useGeneratorHistory('seo-optimizer');
...
const formCompletion = computed(() => {
  const fields = [
    { name: 'Content', filled: !!formData.content.trim() },
    { name: 'Target Keyword', filled: !!formData.targetKeyword.trim() }
  ];
  ...
  try {
    ...
    if (generatedContent.value) {
      addToHistory({
        ...
        preview: generatedContent.value.substring(0, 50) || 'Generated SEO optimization'
      });
    }
