<template>
  ...
  <div v-else class="gmkb-embedded-form">
    ...
    <div v-if="generatedFramework" class="gmkb-embedded-result">
      <div class="gmkb-embedded-result__content" v-html="generatedFramework"></div>
    </div>
...
const canGenerateEmbedded = computed(() => {
  return problem.value?.trim() && approach.value?.trim();
});
...
.gmkb-embedded-result__content :deep(h4) {
  margin: 16px 0 8px 0;
  font-size: 16px;
  font-weight: 600;
  color: #15803d;
...
.gmkb-embedded-result__content :deep(ul),
.gmkb-embedded-result__content :deep(ol) {
  margin: 8px 0;
  padding-left: 24px;
}

.gmkb-embedded-result__content :deep(li) {
  margin: 4px 0;
}
