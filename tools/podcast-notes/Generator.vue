...
const {
  ...
  clearHistory,
  formatTimestamp
} = useGeneratorHistory('podcast-notes');
...
const formCompletion = computed(() => {
  const fields = [
    { name: 'Episode Title', filled: !!episodeTitle.value.trim() },
    { name: 'Topics Covered', filled: !!topicsCovered.value.trim() }
  ];
  ...
  try {
    ...
    if (generatedContent.value) {
      addToHistory({
        ...
        preview: generatedContent.value.substring(0, 50) || 'Generated show notes'
      });
    }
