    ...
    <template #left>
      ...
      <div class="generator__section">
        ...
        <div class="generator__hook-fields">
          <div
            v-for="(field, fieldKey) in AUTHORITY_HOOK_FIELDS"
            :key="fieldKey"
          ...
          >
            ...
            <input
              v-model="hookFields[fieldKey]"
              type="text"
              class="generator__field-input"
              :placeholder="field.placeholder"
              @input="handleFieldChange(fieldKey, $event.target.value)"
            />
      ...
      <div class="generator__actions">
        <!-- Save to Profile (logged-in users) -->
        <button
          v-if="isLoggedIn"
          type="button"
          class="generator__button generator__button--primary"
          :class="{ 'generator__button--loading': isSavingFields }"
          :disabled="!isValid || isSavingFields || !hasSelectedProfile"
          :title="!hasSelectedProfile ? 'Select a profile above to save' : ''"
          @click="handleSaveFields"
        >
          <svg v-if="!isSavingFields" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/>
            <polyline points="17 21 17 13 7 13 7 21"/>
            <polyline points="7 3 7 8 15 8"/>
          </svg>
          {{ isSavingFields ? 'Saving...' : 'Save to Profile' }}
        </button>
        <!-- AI Polish Button -->
        <button
          type="button"
      ...
      <!-- Save Success Notice -->
      <div v-if="showFieldsSaveSuccess" class="generator__save-notice">
...
// Save state for field saving (separate from AI-generated content save)
const isSavingFields = ref(false);
const showFieldsSaveSuccess = ref(false);

// Constants
const totalFields = Object.keys(AUTHORITY_HOOK_FIELDS).length;
...
/**
 * Handle save fields button click (saves raw field values to profile)
 */
const handleSaveFields = async () => {
  isSavingFields.value = true;
  showFieldsSaveSuccess.value = false;

  try {
    const success = await saveHookToProfile();
    if (success) {
      showFieldsSaveSuccess.value = true;
      // Auto-hide after 3 seconds
      setTimeout(() => {
        showFieldsSaveSuccess.value = false;
      }, 3000);
