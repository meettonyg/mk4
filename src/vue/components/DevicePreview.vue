<template>
  <div class="device-preview">
    ...
    <!-- Preview Mode Toggle -->
    <div class="device-preview__divider"></div>
    <button
      :class="['device-preview__btn', 'device-preview__btn--preview', { 'device-preview__btn--active': isPreviewMode }]"
      @click="togglePreview"
      :title="isPreviewMode ? 'Exit Preview Mode' : 'Preview Mode (hide builder UI)'"
    >
      ...
      <span class="device-label">{{ isPreviewMode ? 'Edit' : 'Preview' }}</span>
    </button>
  </div>
</template>
...
// Preview mode state from UI store
const isPreviewMode = computed(() => uiStore.previewMode);
...
.device-preview__btn--preview {
  color: #8b5cf6;
}

.device-preview__btn--preview:hover {
  background: rgba(139, 92, 246, 0.08);
  color: #7c3aed;
}

.device-preview__btn--preview.device-preview__btn--active {
  background: #8b5cf6;
  color: #ffffff;
}
