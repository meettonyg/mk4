...
class Company_Logo_Data_Integration {
    ...
    protected static function debug_log($message) {
        GMKB_Logger::info('[Company_Logo] ' . $message);
    }
    
    /**
     * Load company logo data from native meta
     *
...
GMKB_Logger::startup('Company Logo Data Integration loaded');
...
add_filter('gmkb_enrich_company-logo_props', function($props, $post_id) {
    ...
    if ($logo_data['success']) {
        ...
        GMKB_Logger::debug('Company Logo: Enriched props for post ' . $post_id . ' (has_logo: ' . ($props['has_logo'] ? 'Yes' : 'No') . ')');
    }
