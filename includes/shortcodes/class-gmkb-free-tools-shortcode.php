...
/**
 * Simple nonce shortcode for external SEO tools embedding
 *
 * Usage: [gmkb_nonce]
 *
 * Outputs a hidden script tag with nonce data that can be detected
 * by the seo-tools bundle's getPublicNonce() DOM fallback.
 * Use this on pages that load seo-tools.iife.js externally.
 */
function gmkb_nonce_shortcode() {
    $nonce_data = array(
    ...
    return sprintf(
        '<script type="application/json" id="gmkb-nonce-data">%s</script>',
        wp_json_encode($nonce_data)
    );
}
add_shortcode('gmkb_nonce', 'gmkb_nonce_shortcode');

/**
 * SEO Tools embed shortcode
 *
 * Usage: [gmkb_seo_tools tool="offers"]
 *
 * Embeds the lightweight SEO tools bundle with proper nonce setup.
 * This is an alternative to the full gmkb bundle for simpler embedding.
 */
function gmkb_seo_tools_shortcode($atts) {
    $atts = shortcode_atts(array(
        'tool' => '',
        'mode' => 'embedded',
    ), $atts, 'gmkb_seo_tools');
    ...
    if (empty($tool_slug)) {
        return '<!-- GMKB SEO Tools: No tool specified -->';
    }

    // Enqueue SEO tools bundle
    $version = defined('GMKB_VERSION') ? GMKB_VERSION : '1.0.0';
    $js_file = GMKB_PLUGIN_DIR . 'dist/seo-tools.iife.js';
    $css_file = GMKB_PLUGIN_DIR . 'dist/seo-tools.css';

    if (file_exists($js_file)) {
        wp_enqueue_script(
            'gmkb-seo-tools',
            GMKB_PLUGIN_URL . 'dist/seo-tools.iife.js',
            array(),
            $version,
            true
        );
        ...
        if (file_exists($css_file)) {
            wp_enqueue_style(
                'gmkb-seo-tools',
                GMKB_PLUGIN_URL . 'dist/seo-tools.css',
                array(),
                $version
            );
        }

        // Set global nonce for the SEO tools bundle
        wp_add_inline_script(
            'gmkb-seo-tools',
            'window.gmkbPublicNonce = "' . wp_create_nonce('gmkb_public_ai') . '";' .
            'window.gmkbPublicData = window.gmkbPublicData || {};' .
            'window.gmkbPublicData.publicNonce = "' . wp_create_nonce('gmkb_public_ai') . '";' .
            'window.gmkbPublicData.restUrl = "' . esc_js(rest_url('gmkb/v2/')) . '";',
            'before'
        );
    }

    // Generate container ID
    $container_id = 'gmkb-seo-tool-' . wp_rand(1000, 9999);

    // Output container with data attributes
    return sprintf(
        '<div id="%s" data-mode="%s" data-tool="%s"></div>' .
...
add_shortcode('gmkb_seo_tools', 'gmkb_seo_tools_shortcode');
