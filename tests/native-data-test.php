 ...
 *
 * Phase 0/1 of Native Data Layer Migration
 * @package GMKB
 * @since 3.0.0
 */
...
// Also provide a WP-CLI command for testing
if (defined('WP_CLI') && WP_CLI) {
    WP_CLI::add_command('gmkb native-test', function($args, $assoc_args) {
        $post_id = isset($args[0]) ? absint($args[0]) : 0;

        if (!$post_id) {
            WP_CLI::error('Please provide a post ID: wp gmkb native-test <post_id>');
            return;
        }
        ...
        WP_CLI::log('');
        WP_CLI::log('=== GMKB Native Data Test Results ===');
        WP_CLI::log('');
        WP_CLI::log('Post ID: ' . $results['post_id']);
        WP_CLI::log('Pods Active: ' . ($results['pods_active'] ? 'Yes' : 'No'));
        WP_CLI::log('Timestamp: ' . $results['timestamp']);
        WP_CLI::log('');

        foreach ($results['tests'] as $group_name => $group) {
            WP_CLI::log("--- {$group_name} ---");
            WP_CLI::log("Passed: {$group['passed_count']}/{$group['total_count']}");

            foreach ($group['fields'] as $field_name => $field_data) {
                $status = $field_data['passed'] ? '✅' : '❌';
                $preview = isset($field_data['value']) ?
                    substr(is_string($field_data['value']) ? $field_data['value'] : json_encode($field_data['value']), 0, 50) :
                    '(empty)';
                WP_CLI::log("  {$status} {$field_name}: {$preview}");
            }
            WP_CLI::log('');
        }

        WP_CLI::log('=== Summary ===');
        WP_CLI::log('Total Fields: ' . $results['summary']['total_fields_tested']);
        WP_CLI::log('With Data: ' . $results['summary']['fields_with_data']);
        WP_CLI::log('Empty: ' . $results['summary']['fields_empty']);
        WP_CLI::log('Status: ' . $results['summary']['overall_status']);
        WP_CLI::log('Ready for Phase 2: ' . ($results['summary']['ready_for_phase_2'] ? 'Yes' : 'No'));

        if (!empty($results['summary']['failed_fields'])) {
            WP_CLI::log('');
            WP_CLI::log('Empty fields: ' . implode(', ', $results['summary']['failed_fields']));
        }
    });
}
